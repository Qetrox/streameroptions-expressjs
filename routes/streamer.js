...
router.get('/dashboard', auth.authCookie, streamer.isStreamerSetup, async (req, res) => {
    ...
    database.getPool().query('SELECT * FROM events LEFT JOIN StreamerEvents ON event_id = StreamerEventId AND StreamerEvents.EventStreamerUserId = ?', [req.streamer.streamerUserId], (error, results, fields) => {
        if (error) {
            console.error(error);
        ...
        database.getPool().query('SELECT * FROM customMinecraftEvents WHERE streamerId = ?', [req.streamer.streamerUserId], (error, results, fields) => {
            if (error) {
                console.error(error);
                return res.status(500).send();
            }
...
router.get('/save-modules', auth.authCookie, streamer.isStreamerSetup, async (req, res) => {
    ...
    try {
        ...
        database.getPool().query('insert into StreamerEvents (EventStreamerUserId, StreamerEventId, is_enabled, event_cost) values (?, ?, ?, ?) ON DUPLICATE KEY UPDATE is_enabled = ?, event_cost = ?', [req.streamer.streamerUserId, req.query.module_id, enabled, req.query.price, enabled, req.query.price], (error, results, fields) => {
            if (error) {
                console.error(error);
...
router.get('/save-custom-minecraft-modules', auth.authCookie, streamer.isStreamerSetup, async (req, res) => {
    ...
    const con = mysql.createConnection( // Need to use deprecated connection type due to multiple queries
...
router.post('/setup', auth.authCookie, streamer.isStreamerEnabled, express.urlencoded({ extended: true }), async (req, res) => {
    ...
    try {
        ...
        database.getPool().query('UPDATE streamer SET streamerSlogan = ?, streamerDescription = ? WHERE streamerUserId = ?', [slogan, description, req.user.id], (error, results, fields) => {
            if (error) {
                console.error(error);
                return res.status(500).send();
            }
...
router.post('/activate', auth.authCookie, express.urlencoded({ extended: true }), async (req, res) => {
    ...
    if (confirm === 'Enable') {
        const date = new Date();
        database.getPool().query('INSERT IGNORE INTO streamer (streamerUserId, streamerActiveSince) VALUES (?, ?)', [req.user.id, `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}`], (error, results, fields) => {
            if (error) {
                console.error(error);
                return res.status(500).send();
            }
            ...
            crypto.randomBytes(64, function (ex, buf) {
                database.getPool().query('INSERT IGNORE INTO eventTokens (token, tokenUserId) VALUES (?, ?)', [`${req.user.id}-${buf.toString('hex')}`, req.user.id], (error, results, fields) => {
                    if (error) {
                        console.error(error);
                        return res.status(500).send();
                    }
