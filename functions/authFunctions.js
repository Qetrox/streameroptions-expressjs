...
/**
 * Generate an refresh token for a user
 * @param {object} user - The user object to generate a refresh token for
...
function generateRefreshToken(user, id) {
    ...
    database.getPool().query('insert into refreshTokens values (?, ?) on duplicate key update token = ?', [MD5(token), id, MD5(token)], function (error, results, fields) {
...
function checkRefreshToken(token) {
    database.getPool().query('select * from refreshTokens where token = ?', [MD5(token)], function (error, results, fields) {
